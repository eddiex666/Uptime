...
# Sett opp logging (skriver til konsollen)
...
class Agent(db.Model):
    ...
    port = db.Column(db.Integer, nullable=False, default=8019)  # Default port
...
def request_agent_check(agent_ip, check_type, url, port):
    """Kontakt agent for sjekk."""
    try:
        logging.info(f"[AGENT CHECK] -> IP: {agent_ip}, Type: {check_type}, URL: {url}, Port: {port}")
        ...
        logging.info(f"[AGENT CHECK RESULT] -> {result}")
        return result
    except Exception as e:
        logging.error(f"AGENT CHECK feilet: {e}")
        return "ERROR"

def check_url(url):
    """ Check if a URL is reachable """
    try:
        logging.info(f"[LOCAL CHECK - URL] -> {url}")
...
def check_ping(ip):
    """ Ping en IP """
    logging.info(f"[LOCAL CHECK - PING] -> {ip}")
    response = os.system(f"ping -c 1 {ip} > /dev/null 2>&1")
    return "OK" if response == 0 else "ERROR"

def check_port(ip, port):
    """ Check if a port is open """
    logging.info(f"[LOCAL CHECK - PORT] -> {ip}:{port}")
    try:
...
def check_screen(service_name):
    ...
    logging.info(f"[LOCAL CHECK - SCREEN] -> {service_name}")
    ...
    with app.app_context():
        while True:
            ...
            for site in sites:
                ...
                logging.info(f"=== Checking site: {site.name}, type: {site.check_type}, url: {site.url}, port: {site.port}, old status: {previous_status}")

                try:
                    if site.agent:
                        # AGENT sjekk
                        logging.info(f"[AGENT] -> {site.agent.name} ({site.agent.ip_address}:{site.agent.port}), type: {site.check_type}")
                        site.status = request_agent_check(site.agent.ip_address, site.check_type, site.url, site.port)
                    else:
                        # LOKAL sjekk
                        logging.info(f"[LOCAL] -> type: {site.check_type}, url: {site.url}, port: {site.port}")
                    ...
                    logging.info(f"Result for {site.name} = {site.status}")
                except Exception as e:
                    logging.error(f"Feil under sjekk av {site.name}: {e}")
                    site.status = "ERROR"
                ...
                if site.status != previous_status:
                    ...
                    logging.info(f"STATUS CHANGE: {site.name} from {previous_status} to {site.status}")
