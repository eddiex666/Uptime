...
def request_agent_check(agent_ip, agent_port, check_type, url, port):
    """ Sender en sjekk til agenten på riktig port """
    try:
        logging.info(f"📡 Sending check to agent: {agent_ip}:{agent_port} -> {check_type} {url}:{port if port else ''}")

        response = requests.post(f"http://{agent_ip}:{agent_port}/check", json={
            "auth_key": AUTH_KEY,
        ...
        logging.error(f"❌ ERROR contacting agent {agent_ip}:{agent_port}: {e}")
        return "ERROR"
    ...
    with app.app_context():
        while True:
            ...
            for site in sites:
                ...
                if site.agent:
                    agent_ip = site.agent.ip_address
                    agent_port = site.agent.port  # ✅ Hent agentens port fra databasen!

                    logging.info(f"Checking (AGENT) {site.name} [{site.check_type}] via {site.agent.name} ({agent_ip}:{agent_port})")

                    site.status = request_agent_check(agent_ip, agent_port, site.check_type, site.url, site.port)
